...
Credentials come from the environment or from .deploy.env (gitignored):

    ASC_KEY_ID       the key's 10-character id, the key's 10-character id
...
# scripts/lib/asc.py -> repo root. Draw'n'Cut is a flat Xcode project; there
# is no ios/ subdirectory to descend into.
...
def _es256_signature(signing_input: bytes, key_path: Path) -> bytes:
    """Sign with the .p8 via openssl, returning the raw r||s JWS form.

    openssl emits ASN.1 DER; JWS wants the two 32-byte integers bare. The key
    is passed by path and never read into this process's argv or environment.
    """
    proc = subprocess.run(
    ...
    if proc.returncode != 0:
        raise AscError(
            f"openssl could not sign with {key_path.name}: "
            f"{proc.stderr.decode(errors='replace')[:200]}")
    der = proc.stdout
    # SEQUENCE { INTEGER r, INTEGER s }, each possibly carrying a leading
    # zero byte that keeps DER's two's-complement encoding positive.
    ...
    for _ in range(2):
        if body[offset] != 0x02:
            raise AscError("malformed DER signature from openssl")
...
def token() -> str:
    ...
    if not key_id or not issuer:
        raise AscError(
            ...
            f"{REPO_ROOT / '.deploy.env'}). See .deploy.env.example."
        )
    now = int(time.time())
    ...
    sig = _b64(_es256_signature(signing_input, _key_path(key_id)))
...
def main() -> int:
    ...
    p.add_argument("--bundle-identifier", default="ai.kidwiz.drawncut")
